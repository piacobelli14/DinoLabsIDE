function lintCSS(code) {
    ...
    for (let i = 0; i < code.length; i++) {
        ...
        if (char === "}") {
            if (stack.length === 0) {
                errors.push({ line, col, message: `Unmatched closing '}'` });
            } else {
                ...
                if (last.char !== "{") {
                    errors.push({
                        line,
                        col,
                        message: `Mismatched closing '}' for '${last.char}' from line ${last.line}, col ${last.col}`
                    });
                }
            }
    ...
    if (inComment) {
        errors.push({ line, col, message: "Unclosed block comment" });
    }
