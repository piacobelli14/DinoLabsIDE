function lintHTML(code) {
    ...
    for (let i = 0; i < code.length; i++) {
        ...
        if (!inTag && char === "<") {
            ...
            if (pureTagName) {
                if (isClosing) {
                    if (stack.length === 0) {
                        errors.push({ line, col, message: `Unmatched closing tag </${pureTagName}>` });
                    } else {
                ...
                } else if (!isSelfClosing && !pureTagName.startsWith("!")) {
    ...
    if (stack.length > 0) {
        while (stack.length > 0) {
            ...
            errors.push({ line, col, message: `Unclosed <${unclosed}>` });
        }
    }
