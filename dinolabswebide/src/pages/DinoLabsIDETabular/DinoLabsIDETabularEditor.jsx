  ...
  export default function PinnedHeadersSheet({ fileHandle }) {
        ...
        if (dragData && selectionDrag.originalSelection && selection) {
          ...
          if (shouldAlert) {
            const alertResult = await showDialog({
              title: "System Alert",
              message:
                "The data being dragged over will be replaced. Do you want to continue?",
              showCancel: true
            });
            if (alertResult === null) {
              setDragData(null);
    ...
    const renderDataCell = ({ rowIndex, columnIndex, style }) => {
      ...
      return (
        <div
          style={{
            ...style,
            outline: cellIsActive
              ? "0.2vh solid #008000"
              : "0.2vh solid transparent",
            backgroundColor: cellIsActive
        ...
        >
          <input
            ...
            value={val}
            onChange={(e) =>
              handleEditCell(rowIndex, columnIndex, e.target.value)
            }
            onKeyDown={(e) => {
              if (e.key === "Enter") {
                e.target.blur();
    ...
    useLayoutEffect(() => {
      if (selection && selectionOverlayRef.current) {
        ...
        const newWidth = sumRange(colWidths, selection.left, selection.right + 1);
        const newHeight = sumRange(rowHeights, selection.top, selection.bottom + 1);
              ...
              {({ width, height }) => (
                <Grid
                  ...
                  onScroll={({ scrollLeft: sl, scrollTop: st }) => {
                    ...
                    if (selection && selectionOverlayRef.current) {
                      const newTop = sumRange(rowHeights, 0, selection.top) - st;
                      const newLeft = sumRange(colWidths, 0, selection.left) - sl;
