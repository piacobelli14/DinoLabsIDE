...
export const syntaxHighlight = (
    ...
    const highlightedLines = lines.map((line, index) => {
      ...
      const isTokenFullyInMatch = (start, end) => {
        return matchRanges.some(range => start >= range.start && end <= range.end);
      };
      ...
      lineTokens.forEach(token => {
        ...
        if (token.type && token.type !== 'space') {
          tokenHtml = `<span class="${themeClass} ${token.type}">${escapeHtml(token.value)}</span>`;
        } else {
        ...
        if (isTokenFullyInMatch(tokenStart, tokenEnd)) {
          ...
          overlappingRanges.forEach(range => {
            ...
            if (beforeMatch) {
              if (token.type && token.type !== 'space') {
                newTokenHtml += `<span class="${themeClass} ${token.type}">${escapeHtml(beforeMatch)}</span>`;
              } else {
            ...
            if (matchedText) {
              const matchedHtml = token.type && token.type !== 'space'
                ? `<span class="${themeClass} ${token.type}">${escapeHtml(matchedText)}</span>`
                : escapeHtml(matchedText);
              newTokenHtml += `<span class="searchHighlight">${matchedHtml}</span>`;
            }
            if (afterMatch) {
              if (token.type && token.type !== 'space') {
                newTokenHtml += `<span class="${themeClass} ${token.type}">${escapeHtml(afterMatch)}</span>`;
              } else {
