function lintPython(code) {
    ...
    for (let i = 0; i < code.length; i++) {
        ...
        if (inString) {
            ...
            if (char === "\\") {
                const allowedEscapes = ["'", '"', "\\", "a", "b", "f", "n", "r", "t", "v"];
            ...
            if (inTripleString) {
                if (
                    char === stringChar &&
                    code.substr(i, 3) === stringChar.repeat(3)
                ) {
                    inString = false;
        ...
        if (char === "'" || char === '"') {
            if (code.substr(i, 3) === char.repeat(3)) {
                inString = true;
                inTripleString = true;
                stringChar = char;
                i += 2;
    ...
    for (let i = 0; i < codeLines.length; i++) {
        ...
        if (lineBracketDepth === 0) {
            let trimmed = ln.trim();
            const blockHeaderRegex = /^(if|elif|else|for|while|def|class|try|except|finally|with)\b/;
            if (blockHeaderRegex.test(trimmed) && !trimmed.endsWith(":")) {
                errors.push({
                    line: lineNumber,
                    col: ln.length,
                    message: "Missing colon at end of block header",
                });
            }
        }
    }
