...
function DinoLabsIDEImageEditor({ fileHandle }) {
    ...
    const currentScaleFactor = nativeWidth ? imageWidth / nativeWidth : 1;
    if (imageWidth > MIN_IMAGE_WIDTH) {
      textScaleRef.current = currentScaleFactor;
    }
    const effectiveTextScale = imageWidth <= MIN_IMAGE_WIDTH ? textScaleRef.current : currentScaleFactor;
                        ...
                        <div className="dinolabsIDEMediaCellFlex">
                            ...
                            <Tippy content="Crop Image" theme="tooltip-light">
                                <button
                                    ...
                                    disabled={(isCropDisabled || actionMode === 'Drawing' || actionMode === 'Highlighting' || textOverlays.length > 0) ? true : false}
                                    style={{ "opacity": (isCropDisabled || actionMode === 'Drawing' || actionMode === 'Highlighting' || textOverlays.length > 0) ? "0.6" : "1.0", backgroundColor: isCropping ? "#5C2BE2" : "" }}
                                    className="dinolabsIDEMediaToolButton"
                                >
                            ...
                            if (elem.type === 'drawing') {
                                ...
                                return (
                                    <DraggableTextBox
                                        ...
                                        scaleFactor={effectiveTextScale}
                                    />
                                );
                            }
                            return null;
...
function DraggableTextBox({ overlay, onUpdate, onRemove, onSelect, isSelected, scaleFactor }) {
    ...
    if (scaledStyle.fontSize && scaledStyle.fontSize.endsWith("px")) {
        const size = parseFloat(scaledStyle.fontSize) * effectiveScaleFactor;
        scaledStyle.fontSize = size + "px";
    }
    if (scaledStyle.borderWidth && scaledStyle.borderWidth.endsWith("px")) {
        const bw = parseFloat(scaledStyle.borderWidth) * effectiveScaleFactor;
        scaledStyle.borderWidth = bw + "px";
    }
    if (scaledStyle.borderRadius && scaledStyle.borderRadius.endsWith("px")) {
        const br = parseFloat(scaledStyle.borderRadius) * effectiveScaleFactor;
        scaledStyle.borderRadius = br + "px";
    }
    const scaledPadding = 8 * effectiveScaleFactor;
    const handlerPadding = 4 * effectiveScaleFactor;
    const closeBtnSize = 20 * effectiveScaleFactor;
    const closeBtnFontSize = 16 * effectiveScaleFactor;
    const resizeHandleSize = 8 * effectiveScaleFactor;
